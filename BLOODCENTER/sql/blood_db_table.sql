...
ALTER TABLE donee ADD CONSTRAINT PK_BLOOD_DONEE PRIMARY KEY (
	donee_id
);
...
ALTER TABLE donee ADD CONSTRAINT FK_blood_bank_TO_donee_1 FOREIGN KEY (
	bank_id
)
...
ALTER TABLE blood_keep ADD CONSTRAINT FK_blood_donee_TO_blood_keep_1 FOREIGN KEY (
	donee_id
)
...
insert into blood_donee values ('DE-1','BK01','홍길동','990101-1234567','남','대구 신암1동','010-1111-1111','O형');
...
insert into blood_keep values ('B_KEY-1','BO-1','CT01','BK01',to_date('2025-04-29','YYYY-MM-DD'),to_date('2025-04-29','YYYY-MM-DD')+35,'해당없음',null,null);
...
create or replace view blood_all as
    ...
    bk.bank_id "은행코드",
    bd.donor_id "헌혈자 코드", bd.do_bloodtype "혈액형", bb.bo_donationtype"헌혈종류",
    bb.bo_bloodvolume"헌혈량",
    bk.st_date "보관일자", bk.end_date "만료일자",bk.dis_comment"폐기사유"
